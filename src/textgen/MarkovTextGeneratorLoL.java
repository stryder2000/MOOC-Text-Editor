...
public class MarkovTextGeneratorLoL implements MarkovTextGenerator {
	...
	public void train(String sourceText)
	{
		...
		for(int i=1; i<words.length; i++) {
			...
			if( index != -1 ) {
				...
				ListNode ld = new ListNode(prevWord);
				ld.addNextWord(words[i]);
				wordList.add(ld);
			}
			prevWord = words[i];
		}
		ListNode ld = new ListNode(words[words.length-1]);
		ld.addNextWord(starter);
		wordList.add(ld);
	}
	...
	public String generateText(int numWords) {
		...
		while(wordCount < numWords) {
			...
			String w = wordList.get(index).getRandomNextWord(rnGenerator);
