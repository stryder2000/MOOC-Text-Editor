...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		for(int i=0; i<arr.length; i++) {
			...
			if(arr[i] == 'a' || arr[i]=='e' || arr[i]=='i' || arr[i]=='o' || arr[i]=='u' || arr[i]=='y') {
				count++;
				while(((i+1)<arr.length) && (arr[i+1] == 'a' || arr[i+1]=='e' || 
						arr[i+1]=='i' || arr[i+1]=='o' || arr[i+1]=='u' || arr[i+1]=='y') ) {
					++i;
					if(i == arr.length-1) {
						return count;
					}
	...
	public double getFleschScore()
	{
		...
		float fleschScore = 206.835f - 1.105f*(numWords/numSentences) - 84.6f*(numSyllables/numWords);
